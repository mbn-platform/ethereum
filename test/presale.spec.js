...
module.exports = ({describe, define, before, after, it}) => {
  describe('Presale', function() {
    ...
    define(async ({accounts, contracts, toWei}) => {
      ...
      const presale = await contracts.presale.deploy(
        main,
        main,
      ...
      const token = await contracts.token.deploy(presale.options.address)
      .send(main);

      await presale.methods.setToken(token.options.address).send(main);

      return {presale, token, releaseTime, unlockTime};
    });

    describe('#setNextRate() & #getRate()', () => {
      it(
        'Should set next rate',
        async ({accounts, presale, evm}) => {
          const {main} = accounts;
          const {getCurrentRate, setNextRate} = presale.methods;
    ...
    describe('#addStage()', () => {
      it(
        ...
        async ({accounts, presale, releaseTime}) => {
          const {main} = accounts;
          const {
            addStage,
            getStagesLength,
    ...
    describe('Reseller management', () => {
      describe('#addReseller()', () => {
        it(
          ...
          async ({presale, accounts}) => {
            const {main, member1} = accounts;
            const {addReseller, isReseller} = presale.methods;
        ...
        it(
          ...
          async ({presale, accounts}) => {
            const {main, member1} = accounts;
            const {addReseller} = presale.methods;
      ...
      describe('#removeReseller()', () => {
        ...
        it(
          ...
          async ({presale, accounts}) => {
            const {main, member2} = accounts;
            const {removeReseller, isReseller} = presale.methods;
    ...
    describe('#getCurrentRate()', () => {
      it(
        ...
        async ({accounts, presale}) => {
          const {main} = accounts;
          const {getTokensAmount, getCurrentRate} = presale.methods;
    ...
    describe('#transferTokens()', () => {
      it(
        ...
        snapshot,
        async ({accounts, presale, token, web3}) => {
          const {main, member1, user1} = accounts;
          const {transferTokens} = presale.methods;
          ...
          should(balanceOfAfter).be.equal('2');
        },
        rollback
      );
      ...
      it(
        ...
        async ({accounts, presale, token, web3}) => {
          const {main, member1, user1} = accounts;
          const {transferTokens, getAvailable} = presale.methods;
      ...
      it(
        ...
        async ({accounts, presale, token, web3}) => {
          const {main, member1, user1} = accounts;
          const {transferTokens, getAvailable, getLocksCount} = presale.methods;
          const {balanceOf} = token.methods;
